...
public class FlyBehaviour : GenericBehaviour
{
    ...
    private bool fly; // Boolean to determine whether or not the player activated fly mode.
    ...
    private void Update()
    {
                                           ...
                                           && !behaviourManager.GetTempLockStatus(behaviourManager.GetDefaultBehaviour))
        {
            ...
            if (fly)
            {
                ...
                behaviourManager.RegisterBehaviour(behaviourCode);
            }
            else
            {
                ...
                behaviourManager.UnregisterBehaviour(behaviourCode);
            }
        }

        // Assert this is the active behaviour
        fly = fly && behaviourManager.IsCurrentBehaviour(behaviourCode);
    ...
    private void FlyManagement(float horizontal, float vertical)
    {
        ...
        behaviourManager.GetRigidBody.AddForce(
            direction * flySpeed * 100 * (behaviourManager.IsSprinting() ? sprintFactor : 1), ForceMode.Acceleration);
    }
