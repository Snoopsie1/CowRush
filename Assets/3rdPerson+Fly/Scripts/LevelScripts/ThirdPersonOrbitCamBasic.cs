...
public class ThirdPersonOrbitCamBasic : MonoBehaviour
{
    ...
    public Vector3 pivotOffset = new(0.0f, 1.7f, 0.0f); // Offset to repoint the camera.
    public Vector3 camOffset = new(0.0f, 0.0f, -3.0f); // Offset to relocate the camera related to the player position.
    ...
    private float angleV; // Float to store camera vertical angle related to mouse movement.
    ...
    private void Update()
    {
        ...
        var camYRotation = Quaternion.Euler(0, GetH, 0);
        var aimRotation = Quaternion.Euler(-angleV, GetH, 0);
        ...
        var customOffsetCollision = isCustomOffset && noCollisionOffset.sqrMagnitude < targetCamOffset.sqrMagnitude;
    ...
    private bool ViewingPosCheck(Vector3 checkPos)
    {
        ...
        var target = player.position + pivotOffset;
        var direction = target - checkPos;
        // If a raycast from the check position to the player hits something...
        if (Physics.SphereCast(checkPos, 0.2f, direction, out var hit, direction.magnitude))
    ...
    private bool ReverseViewingPosCheck(Vector3 checkPos)
    {
        ...
        var origin = player.position + pivotOffset;
        var direction = checkPos - origin;
        if (Physics.SphereCast(origin, 0.2f, direction, out var hit, direction.magnitude))
